...
pub fn cli() -> Command {
    Command::new("readme")
        .about("Update or create README.md with server info")
}

// TODO: pretty print the tables...

pub static NOTICE: &str = "";
pub static SERVERINFO_REGEX: &str = r"(<!--start:mcman-server-->)([\w\W]*)(<!--end:mcman-server-->)";
...
pub async fn run() -> Result<()> {
    ...
    let server_info_text = {
        ...
        let text = "| ".to_owned() + &line1.join(" | ") + " |"
            + "\n"
            + "| " + &line1.iter().map(|_| "---").collect::<Vec<&str>>().join(" | ") + " |"
            + "\n"
            + "| " + &line2.join(" | ") + " |\n";

        SERVERINFO_START.to_owned()
            + NOTICE + "\n" + &text + SERVERINFO_END
    };
    ...
    let stage1 = serv_regex.replace_all(&readme_content, |_caps: &regex::Captures| {
        server_info_text.clone()
    }).into_owned();

    let addon_regex = Regex::new(ADDONS_REGEX).unwrap();

    let stage2 = addon_regex.replace_all(&stage1, |_caps: &regex::Captures| {
        addon_list_text.clone()
    });
...
pub fn initialize_readme(
    ...
    let readme_content = readme_content
        ...
        .replace("{ADDON_HEADER}", match server.jar {
            Downloadable::Quilt { .. }
            | Downloadable::Fabric { .. } => "Mods",
            _ => "Plugins",
        });
