    ...
    pub fn get_metadata(&self) -> IndexMap<Cow<'static, str>, String> {
        ...
        match self {
            Self::Fabric { loader, installer } | Self::Quilt { loader, installer } => {
                map.insert("Loader".into(), version_id!(loader));

                if installer != "latest" {
                    map.insert("Installer".into(), format!("`{installer}`"));
                }
            },
            ...
            Self::PaperMC { build, .. } | Self::Purpur { build } => {
                map.insert("Build".into(), version_id!(build, |id| "`#{id}`"));
            },
            ...
            Self::Downloadable { inner } => match inner {
                Downloadable::Jenkins {
                    ...
                    if artifact != "first" {
                        map.insert("Artifact".into(), format!("`{artifact}`"));
                    }
                },

                Downloadable::GithubRelease { tag, asset, .. } => {
                    map.insert("Release".into(), version_id!(tag));

                    if asset != "first" {
                        map.insert("Asset".into(), format!("`{asset}`"));
                    }
                },
