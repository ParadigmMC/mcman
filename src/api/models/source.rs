...
use super::{
    mrpack::resolve_mrpack_addons, packwiz::resolve_packwiz_addons, Addon, AddonListFile,
    ModpackType,
};
use crate::api::{
    app::App,
    models::ModpackSource,
    utils::{accessor::Accessor, fs::with_extension_if_none, toml::read_toml},
};
...
impl FromStr for SourceType {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        let (ty, val) = s.split_once(':').ok_or(anyhow!(
            "Source identifier are in the format of '<type>:<path or url>'"
        ))?;

        Ok(match ty {
            "file" | "f" => SourceType::File { path: val.into() },
            "packwiz" | "pw" => SourceType::Modpack {
                modpack_source: ModpackSource::from_str(val)?,
                modpack_type: ModpackType::Packwiz,
            },
            "mrpack" => SourceType::Modpack {
                modpack_source: ModpackSource::from_str(val)?,
                modpack_type: ModpackType::MRPack,
            },
...
impl Source {
    ...
    pub fn accessor(&self, relative_to: &Path) -> Result<Accessor> {
        match &self.source_type {
            SourceType::File { path } => Ok(Accessor::Local(
                relative_to
                    .join(path)
                    .canonicalize()
                    .with_context(|| format!("Resolving path: {:?}", relative_to.join(path)))?,
            )),
            SourceType::Folder { path } => Ok(Accessor::Local(
                relative_to
                    .join(path)
                    .canonicalize()
                    .with_context(|| format!("Resolving path: {:?}", relative_to.join(path)))?,
            ...
            SourceType::Modpack {
                modpack_source: modpack,
                ..
            } => modpack
                .accessor(relative_to)
        ...
        match &self.source_type {
            SourceType::File { path } => {
                ...
                let file: AddonListFile =
                    read_toml(&path).with_context(|| format!("Source: File => {path:?}"))?;
                Ok(file.flatten())
            },
