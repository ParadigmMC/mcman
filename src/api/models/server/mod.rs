...
use super::{
    launcher::ServerLauncher, markdown::MarkdownOptions, mrpack::resolve_mrpack_serverjar,
    packwiz::resolve_packwiz_serverjar, ModpackType, Source, SourceType,
};
...
impl Server {
    ...
    pub fn get_arguments(&self) -> Vec<String> {
        ...
        if self.launcher.eula_args
            && self
                .jar
                .as_ref()
                .is_some_and(|x| x.flavor().supports_eula_args())
        {
        ...
        if self.launcher.nogui
            && self
                .jar
                .as_ref()
                .is_some_and(|x| x.flavor().supports_nogui())
        {
